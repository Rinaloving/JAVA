...
public class ShowWallPaperAction extends ActionSupport
{
  ...
  public InputStream getDownloadFile()
    throws Exception
  {
    this.FilePath = ServletActionContext.getServletContext().getRealPath("/wallpaper");

    File file = new File(this.FilePath);

    if (file.exists()) {
      System.out.println("2222222222/wallpaper");
      this.names = file.listFiles();
      for (int i = 0; i < this.names.length; i++)
      {
        this.realFilePath = ("/wallpaper/" + this.names[i].getName().trim().toString());
    ...
    return ServletActionContext.getServletContext().getResourceAsStream("/wallpaper/" + this.fileName2);
  }
